...
def initial_record():
    if os.path.exists('./output1/results.txt'):
        os.remove('./output1/results.txt')
    with open('./output1/results.txt','w') as f:    
        f.write("Epoch,Time(s),Loss,F1,Accuracy\n")   
...
def train_fg(model, optim, loss, features, labels, train_g, test_g, test_mask,
    ...
    for epoch in range(n_epochs):
        ...
        metric_f1, metric_acc = evaluate(model, train_g, features, labels, device)
        print("Epoch {:05d}, Time(s) {:.4f}, Loss {:.4f}, F1 {:.4f} ".format(
                epoch, np.mean(duration), loss_val, metric_f1))
        
        epoch_result = "{:05d},{:.4f},{:.4f},{:.4f}\n".format(epoch, np.mean(duration), loss_val, metric_f1)
        with open('./output1/results.txt','a+') as f:    
    ...
    if compute_metrics:
        acc, f1, p, r, roc, pr, ap, cm = get_metrics(class_preds, pred_proba, labels.numpy(), test_mask.numpy(), './output1/')
        print("Metrics")
...
def get_f1_score(y_true, y_pred):
    ...
    acc=(cf_m[0,0] + cf_m[1,1])/ (cf_m[0,0] + cf_m[1,1] + cf_m[0,1] + cf_m[1,0])
...
def evaluate(model, g, features, labels, device):
    ...
    precision, recall, f1, acc = get_f1_score(labels, preds)
